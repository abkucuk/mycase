...
public class OrderRepositoryWithBagRelationshipsImpl implements OrderRepositoryWithBagRelationships {
    ...
    public Optional<Order> fetchBagRelationships(Optional<Order> order) {
        return order.map(this::fetchBooks);
    }
    ...
    public List<Order> fetchBagRelationships(List<Order> orders) {
        return Optional.of(orders).map(this::fetchBooks).orElse(Collections.emptyList());
    }

    Order fetchBooks(Order result) {
        return entityManager
            .createQuery("select order from Order order left join fetch order.books where order is :order", Order.class)
            .setParameter("order", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Order> fetchBooks(List<Order> orders) {
        ...
        List<Order> result = entityManager
            .createQuery("select distinct order from Order order left join fetch order.books where order in :orders", Order.class)
